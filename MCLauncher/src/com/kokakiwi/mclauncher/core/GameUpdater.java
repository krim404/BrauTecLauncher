...
public class GameUpdater implements Runnable
{
    ...
    public void run()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < addList.size(); i++)
            {
                ...
                {
                	configUrl = new URL(addList.get(i));
                }
            }
            ...
            final File modfolder = new File(Utils.getWorkingDirectory(launcherFrame)
                    .getAbsolutePath() + File.separatorChar, "mods");
            
            final File absolute = new File(Utils.getWorkingDirectory(launcherFrame)
                    .getAbsolutePath());
            ...
            boolean repair = launcherFrame.getConfig().getString(
                    "repair") == null ? false : true;
            ...
            final File downloading = new File(dir, "downloading.tmp");
            
            final File conffolder = new File(Utils.getWorkingDirectory(launcherFrame)
                    .getAbsolutePath() + File.separatorChar, "config");
            ...
            if (useCustomVersion)
            {
                ...
                if (customVersionFile.exists())
                {
                    ...
                    if(latestCustomVersion.toString().contains("c"))
                    {
                    	deleteconfig = true;
                    }
                }
            ...
            {
            	FileWriter fstream = new FileWriter(downloading);
            	BufferedWriter out = new BufferedWriter(fstream);
            	out.write("downloading bdh mc...");
            	out.close();
            	
            	deleteDir(modfolder);
            	
            	if(deleteconfig == true)
            		deleteDir(conffolder);
                ...
                if(configUrl != null)
                {
	                final String fileName = getFileName(configUrl);
	                final File file = new File(
	                        Utils.getWorkingDirectory(launcherFrame)
	                                + File.separator + File.separator + "bin" + File.separatorChar
	                                + fileName);
	                if(file.exists() && !file.isDirectory() && fileName.length() > 0)
	                {
	                	extract(file, Utils.getPrimWorkingDirectory(launcherFrame).getParent() + File.separator);
	                }
                }
                extractJars(path);
    ...
    private void downloadFiles(String path) throws Exception
    {
        ...
        final int downloadNum = jarUrls.length + additionalsUrls.length + 2;
